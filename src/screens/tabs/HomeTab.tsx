...
export function HomeTab() {
  ...
  const overdueCount = items.filter(
    (i) => calculateDaysLeft(i.expiresAt) < 0,
  ).length;
  ...
  if (totalCount === 0) {
    ...
    stateDesc =
      "AI 렌즈로 영수증이나 냉장고 내부를 촬영하여 식재료를 채워보세요.";
    ...
    stateDesc =
      "상태를 신속하게 확인하고 폐기 여부를 결정하거나 즉시 소진해보세요.";
  } else if (soonCount > 0) {
    ...
    stateDesc =
      "오늘 또는 내일 내에 사용할 재료들이 있습니다. 추천 레시피로 소진해보세요.";
  }
  ...
  for (let i = startDayOfWeek - 1; i >= 0; i--) {
    prevMonthDays.push(
      new Date(prevMonthYear, prevMonth, totalDaysInPrevMonth - i),
    );
  }
  ...
  const getDayTone = (expiringItems: InventoryItem[]) => {
    ...
    const hasOverdue = expiringItems.some(
      (item) => calculateDaysLeft(item.expiresAt) < 0,
    );
    ...
    <div className="grid gap-5">
      ...
      <section className="grid grid-cols-3 gap-3 my-0.5" aria-label="홈 지표">
        ...
        <MetricCard
          ...
          helper={
            overdueCount > 0 ? `기한초과 ${overdueCount}개` : "오늘 확인 필요"
          }
      ...
      <Panel
        eyebrow="Calendar"
        title={`${currentMonth + 1}월 소비기한 캘린더`}
        description="보관 중인 식재료의 소비기한 분포를 달력에서 한눈에 확인하세요."
      >
            ...
            {allCalendarDays.map((date, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={cn(
                    "relative flex flex-col items-center justify-between min-h-[56px] p-1 rounded-lg transition-all duration-300",
                    !isCurrentMonth && "opacity-30",
                    isToday
                      ? "bg-[var(--color-surface-brand-soft)]"
                      : "bg-[var(--color-bg-base)]",
                    tone === "danger" &&
                      "bg-[var(--color-surface-danger-soft)]/20",
                    tone === "soon" &&
                      "bg-[var(--color-surface-warning-soft)]/20",
                  )}
                >
                  <span
                    className={cn(
                    ...
                    <div className="flex flex-col items-center gap-0.5 w-full">
                      <span
                        className={cn(
                          ...
                          tone === "danger" &&
                            "bg-[var(--color-error)] text-[var(--color-on-secondary)]",
                          tone === "soon" &&
                            "bg-[var(--color-primary)] text-[var(--color-on-primary)]",
                          tone === "safe" &&
                            "bg-[var(--color-success)] text-[var(--color-on-secondary)]",
                        )}
                      >
